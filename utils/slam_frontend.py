...
class FrontEnd(mp.Process):
    ...
    def run(self):
        ...
        while True:
            try:
                data_vis2main = self.q_vis2main.get_nowait()
            except queue.Empty:
                data_vis2main = None

            if data_vis2main is not None:
            ...
            if data is not None:
                if data[0] == "sync_backend":
                    ...
                    self.requested_keyframe -= 1
                    Log(
                        f"Frontend received keyframe sync. pending={self.requested_keyframe}"
                    )

                elif data[0] == "init":
                    self.sync_backend(data)
                    self.requested_init = False
                    Log("Frontend received init sync")
