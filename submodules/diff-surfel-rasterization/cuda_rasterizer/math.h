...
// Minimal float vector operators needed by mat33/mat34/mat44/SE3/SO3.
// auxiliary.h already defines: float3-, float3*, float2*, float3-float3, float2-float2
// We only add the ones auxiliary.h does NOT provide.

// float3 addition
__forceinline__ __host__ __device__ float3 operator+(float3 a, float3 b){
    return make_float3(a.x+b.x, a.y+b.y, a.z+b.z);
}
// float3 scalar multiply (right)
