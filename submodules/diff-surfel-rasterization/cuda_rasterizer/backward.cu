...
__device__ void computeColorFromSH(int idx, int deg, int max_coeffs, const glm::vec3* means, glm::vec3 campos, const float* shs, const bool* clamped, const glm::vec3* dL_dcolor, glm::vec3* dL_dmeans, glm::vec3* dL_dshs, float* dL_dtau)
{
	...
	// SH view-direction contribution to camera pose gradient.
	// The view direction from camera to Gaussian is (p_w - campos).
	// If camera translates by rho, campos changes by rho, so d(dir)/d_rho = -I (unnormalized).
	// After normalization the gradient flows through dnormvdv, giving dL_dmean.
	// From camera's perspective: dL/d_rho += -dL/d_mean_world (translation shifts camera = negated)
	if (dL_dtau != nullptr) {
		atomicAdd(&dL_dtau[0], -dL_dmean.x);
		atomicAdd(&dL_dtau[1], -dL_dmean.y);
		atomicAdd(&dL_dtau[2], -dL_dmean.z);
	}
}
	...
	float* dL_dtau)
{
	...
	// -----------------------------------------------------------------------
	// Pose gradient: dL/d_tau = (dL/d_rho [3], dL/d_theta [3])
	// Camera pose perturbs p_c = R_cw * p_w + t_cw via left-multiplication
	// exp(xi) on T_cw. Chain rule through the full projection T = M^T * P_cam.
	//
	// dL/d(p_c) from two sources:
	//  1. From dL_dM[2] (gradient w.r.t. world position of surfel center):
	//     dL/d(p_w) = dL_dM[2]. We rotate to camera space: dL/d(p_c) = R_cw * dL/d(p_w)
	//     But we actually need the full projection contribution. Instead we compute
	//     dL/d(p_c) by going through the projection Jacobian of the surfel center column.
	//
	// The surfel center contribution: T[2] = [p_w^T 1] * P_cam
	// dT[2]/d(p_w) = P_cam[0:3, :] (upper 3 rows of P_cam, 3x3)
	// dL/d(p_w)_from_T = dL_dT[2] (already in dL_dM[2] via M backward)
	//
	// For translation rho (camera frame): dp_c/d_rho = I
	//   dL/d_rho += dL/d(p_c) dot I = dL/d(p_c)
	// For rotation theta (so3): dp_c/d_theta_i = -[p_c]_x e_i = e_i cross p_c
	//   dL/d_theta += dL/d(p_c) dot (dp_c/d_theta)
	//
	// dL/d(p_c) = J_proj^{-T} * dL/d(p_w) where J_proj = d(p_w)/d(p_c) = R_wc = R_cw^T
